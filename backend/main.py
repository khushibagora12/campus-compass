...
import time
import redis # <-- NEW: Import redis directly
...
from ingest import process_documents_for_college
...
# --- APP SETUP ---
app = FastAPI(title="Campus Compass API", version="4.0.0 Final")
...
# NEW: Create a direct Redis client
redis_client = redis.from_url(REDIS_URL, decode_responses=True)
print("Services initialized.")

# --- PROMPT SETUP ---
prompt_template = """
You are a helpful assistant for college students named Campus Compass... (Your full prompt here)
...
# --- STUDENT API ENDPOINT (WITH MANUAL REDIS LOGIC) ---
...
async def ask_question(request: QueryRequest):
    ...
    try:
        # --- NEW: Manual Redis Chat History ---
        session_key = f"chat_history:{request.session_id}"
        chat_history = redis_client.get(session_key) or ""

        # --- TANAY'S DIRECT RETRIEVAL LOGIC ---
        ...
        answer = rag_chain.invoke({
            "context": context, "question": request.query, "chat_history": chat_history
        })
        
        # --- NEW: Manual saving of chat history to Redis ---
        new_history = f"{chat_history}\nHuman: {request.query}\nAI: {answer}"
        redis_client.set(session_key, new_history, ex=3600) # Expire after 1 hour

        # --- HUMAN FALLBACK LOGIC ---
        if "I_DO_NOT_KNOW" in answer or not context.strip():
            fallback_message = "I couldn't find a specific answer..." # Your fallback message
